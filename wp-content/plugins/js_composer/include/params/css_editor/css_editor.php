	...
	class WPBakeryVisualComposerCssEditor {
		...
		protected function layerControls( $name, $prefix = '' ) {
			...
			foreach ( $this->positions as $pos ) {
				$k = $name . '-' . $pos . ( '' !== $prefix ? '_' . $prefix : '' );
				if( $name == 'border' )
					$k = 'border-'.$pos.'-width';

				$new_value = $this->value[$k];

				$output .= '<input type="text" name="' . $name . '_' . $pos . ( '' !== $prefix ? '_' . $prefix : '' ) . '" data-name="' . $name  . '-' . $pos . ( '' !== $prefix ? '-' . $prefix : '' ) . '" class="vc_' . $pos . '" placeholder="-" data-attribute="' . $name . '" value="'.$new_value.'">';
			}
