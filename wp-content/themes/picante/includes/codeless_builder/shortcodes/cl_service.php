		...
		$output .= '<div class="wrapper-form" '.$this->generateStyle('.cl_service > .icon_wrapper .wrapper-form').'>';
			...
			if( $media == 'type_icon' )
				...
				if( $media == 'type_svg' ){
					...
					$output .= '<svg perspectiveAspectRatio="xMinYMax meet" '.$this->generateStyle('.cl_service > .icon_wrapper svg').'  class="icon-svg '.$animated.' cl-svg-'.str_replace('cl-icon-', '', $icon).'"><use xlink:href="'.esc_url( get_template_directory_uri().'/css/fonts/codeless-svg-defs.svg'.'#cl-svg-'.str_replace('cl-icon-', '', $icon) ).'"></use></svg>';
				}
			else
				if( $media == 'type_custom' ){
					...
					$output .= '<img src="' . codeless_get_attachment_image_src( $img_id, $image_size ) .'" alt="'.esc_attr__( 'Custom Image', 'picante' ).'" />';
