	...
	class CodelessImageSizes {
		...
		public static function addPage() {
			if( function_exists( 'codeless_add_submenu_page' ) )
				codeless_add_submenu_page(
					'codeless-panel',
					esc_html__( 'Image Sizes', 'picante' ),
					esc_html__( 'Image Sizes', 'picante' ),
					'administrator',
		...
		public static function createPage() {
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Image Sizes', 'picante' ); ?></h2>

				<p><?php esc_html_e( 'In this Options Section you can define all image sizes of site. Leave the width and height empty to display the full image. Leave empty the height attribute to disable the crop.', 'picante' ); ?></p>
				...
				<h2 class="nav-tab-wrapper codeless-image-sizes-admin-tabs" style="margin-top:20px;">
					...
					$tabs = apply_filters( 'codeless_image_sizes_tabs', array(
						'general' => esc_html__( 'General', 'picante' ),
						'blog'    => esc_html__( 'Blog', 'picante' ),
						'portfolio'    => esc_html__( 'Portfolio', 'picante' ),
						'team'    => esc_html__( 'Team', 'picante' ),
					) );

					
					$tabs['other'] = esc_html__( 'Other', 'picante' );
				...
				<form method="post" action="options.php">
					...
					<table class="form-table codeless-image-sizes-admin-table">
						<tr valign="top" data-codeless-section="general">
							<th scope="row"><?php esc_html_e( 'Image Resizing', 'picante' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input id="codeless_optimize_image_resizing" type="checkbox" name="codeless_image_sizes[optimize_image_resizing]" <?php checked( codeless_get_mod( 'optimize_image_resizing', true ) ); ?>>
											<?php esc_html_e( 'Optimized Image Sizes', 'picante' ); ?>
						...
						<tr valign="top" data-codeless-section="general">
							<th scope="row"><?php esc_html_e( 'Add Custom Size', 'picante' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input id="cl_custom_img_sizes" type="text" name="codeless_image_sizes[cl_custom_img_sizes]" value="">
									
											<p class="description"><?php esc_html_e( 'Here you can add a new id for a custom image size. You can edit width, height after Save. The new size will be shown at Other Section. After setting the new image size you can use it over your webpage.', 'picante' ); ?></p>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						foreach ( $sizes as $size => $args ) : ?>
							...
							<tr valign="top" data-codeless-section="<?php echo esc_attr( $section ); ?>" style="display:none;">
								...
								<td>
									<label for="<?php echo esc_attr( $width ); ?>"><?php esc_html_e( 'Width', 'picante' ); ?></label>
									...
									<label for="<?php echo esc_attr( $height ); ?>"><?php esc_html_e( 'Height', 'picante' ); ?></label>
									...
									<label for="<?php echo esc_attr( $crop ); ?>"><?php esc_html_e( 'Crop Location', 'picante' ); ?></label>
						...
						<tr valign="top" data-codeless-section="other" style="display:none;">
								<p><?php esc_html_e('Here will show all custom image sizes added by you. Please add a new custom image size at General Section.', 'picante' ) ?></p>
							</tr>
