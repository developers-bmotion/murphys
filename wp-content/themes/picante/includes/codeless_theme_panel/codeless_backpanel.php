...
private static function let_to_num( $size ) {
     ...
     */
     ...
     public function template_warnings() {
        ...
        if ( $incorrect_max_execution_time || $incorrect_max_input_time || $incorrect_memory_limit || $incorrect_upload_max_filesize) {
            ...
            echo '<br><strong>'.esc_html__('Please resolve these issues before installing any template.', 'picante').'</strong>';
            echo '<ol>';
            if ($incorrect_max_execution_time) {
                echo '<li><strong>'.esc_html__('Maximum Execution Time', 'picante' ).' (max_execution_time) : </strong>' . $max_execution_time . ' seconds. <span style="color:red"> '.esc_html__('Recommended max_execution_time should be at least 60 Seconds.', 'picante').'</span></li>';
            }
            if ($incorrect_max_input_time) {
                echo '<li><strong>'.esc_html__('Maximum Input Time', 'picante').' (max_input_time) : </strong>' . $max_input_time . ' seconds. <span style="color:red"> '.esc_html__('Recommended max_input_time should be at least 60 Seconds.', 'picante' ).'</span></li>';
            }
            if ($incorrect_memory_limit) {
                echo '<li><strong>'.esc_html__('WordPress Memory Limit', 'picante' ).' (WP_MEMORY_LIMIT) : </strong>' . WP_MEMORY_LIMIT . ' <span style="color:red"> '.esc_html__('Recommended memory limit should be at least 128MB.', 'picante' ).' <a target="_blank" href="http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP">'.esc_html__('Increasing memory allocated to PHP', 'picante').'</a></span></li>';
            }
            if ($incorrect_upload_max_filesize) {
                echo '<li><strong>'.esc_html__('Maximum Upload File Size', 'picante' ).' (upload_max_filesize) : </strong>' . $upload_max_filesize . ' <span style="color:red"> '.esc_html__('Recommended Maximum Upload Filesize should be at least 10MB.', 'picante' ).'</li>';
            }
    ...
    public static function cl_templates(){
        ...
        $request = wp_remote_get('http://codeless.co/picante/register/login.php?check=1&code='.$purchase_code);
    ...
    public static function cl_removeproduct(){
        ...
        $request = wp_remote_get('http://codeless.co/picante/register/login.php?remove=1&code='.$purchase_code);
    ...
    public static function cl_version(){
        ...
        $raw_response = wp_remote_get('http://codeless.co/picante/register/update.php');
    ...
    */
    ...
    public static function cl_announcements(){
            ...
            $raw_response = wp_remote_get('http://codeless.co/picante/register/announcements.php');
    ...
    static function get_demos(){
        $demos = array(
            array(
                'id' => 'default',
                'label' => 'Default',
                'preview' => 'http://codeless.co/picante/default',
                'parts' => 1
            ),
            array(
                'id' => 'organic',
                'label' => 'Organic',
                'preview' => 'http://codeless.co/picante/organic',
                'parts' => 1
            ),
            array(
                'id' => 'coffee',
                'label' => 'Coffee',
                'preview' => 'http://codeless.co/picante/coffee',
                'parts' => 1
            ),
            array(
                'id' => 'blog',
                'label' => 'Blog',
                'preview' => 'http://codeless.co/picante/blog',
                'parts' => 1
            ),
            ...
            array(
                'id' => 'pizza',
                'label' => 'Pizza',
                'preview' => 'http://codeless.co/picante/pizza',
                'parts' => 1
            ),
            ...
            array(
                'id' => 'reservation',
                'label' => 'Reservation',
                'preview' => 'https://codeless.co/picante/default/?page_id=584',
                'parts' => 1
            ),
            array(
                'id' => 'tasty',
                'label' => 'Tasty',
                'preview' => 'https://codeless.co/picante/default/?page_id=665',
                'parts' => 1
            ),
    ...
    static function get_headers(){
        $demos = array(
            ...
            array(
                'id' => 'main5',
                'label' => 'Coffee Overlay',
                'preview' => 'http://codeless.co/picante/coffee'
            ),
            array(
                'id' => 'main6',
                'label' => 'Top Navigation + Center Logo',
                'preview' => 'http://codeless.co/picante/blog'
            ),
        );
        return $demos;
